...
void Solver::Init(const SolverParameter& param) {
  ...
  kb_weight_ = Context::get_double("kb_weight");
}
...
void Solver::Update() {
  ...
  for (; d_idx < data_idx_end; ++d_idx) {
    ...
    BOOST_FOREACH(const UIntFloatPair& n_new_ele, n_new) {
      ...
      // KB constraints
      cur_log_weight -= AddKBConstraints(datum->data(), vertex->kb_word_reg_weight());
...
float Solver::AddKBConstraints(const UIntFloatMap& data, const UIntFloatMap& vertex_kb_w) {
  float result = 0.0;
  BOOST_FOREACH(const UIntFloatPair& ele, data) {
    if (vertex_kb_w.find(ele.first) != vertex_kb_w.end()) {
      result += ele.second * vertex_kb_w.at(ele.first);
    }
  }
  result *= kb_weight_;
  return result;
