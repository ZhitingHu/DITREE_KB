...
void DITreeEngine::ReadKB(const map<int, string>& vocab) {
  LOG(INFO) << "Reading KB.";
  const string& kb_file = Context::get_string("kb_data");

  map<string, int> word_index_map;
  for (auto const &item : vocab) {
    word_index_map[item.second] = item.first;
  }

  fstream input(kb_file.c_str(), ios::in);
  CHECK(input.is_open()) << "File not found: " << kb_file;
  string parent_word, child_word;
  while (input >> parent_word >> child_word) {
    if (word_index_map.find(parent_word) == word_index_map.end() ||
        word_index_map.find(child_word) == word_index_map.end()) {
        continue;
    }
    const uint32 parent_word_idx = word_index_map[parent_word];
    const uint32 child_word_idx = word_index_map[child_word];
    if (kb_.find(parent_word_idx) == kb_.end()) {
        kb_[parent_word_idx] = vector<uint32>();
    }
    kb_[parent_word_idx].push_back(child_word_idx);
  }
  input.close();

  //// for test
  //for (auto const &item : kb_) {
  //  for (auto const &a : item.second) {
  //    LOG(ERROR) << vocab.at(item.first) << " " << vocab.at(a);
  //  }
  //}
  //LOG(FATAL) << "Done.";
