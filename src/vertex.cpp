...
void Vertex::RecursComputeKBWeight() {
  ComputeKBWeight();

  // recursion: from root to leaves
  // and from left to right
  for (int c_idx = 0; c_idx < children_.size(); ++c_idx) {
    children_[c_idx]->RecursComputeKBWeight();
  }
}

void Vertex::ComputeKBWeight() {
  const UIntVecUIntMap& kb = Context::kb();
  for (auto const &item : kb) {
    kb_word_reg_weight_[item.first] = 0;
    kb_word_reg_weight_for_child_[item.first] = 0;
  }
  CHECK(parent_ != NULL);

  const auto& parent_kb_word_reg_weight = parent_->kb_word_reg_weight_for_child();
  for (auto const &item : parent_kb_word_reg_weight) {
    kb_word_reg_weight_[item.first] += item.second;
  }
  // for child
  for (auto const &item : kb_word_reg_weight_) {
    kb_word_reg_weight_for_child_[item.first] += item.second; 
    for (auto const &kb_child_word_idx : kb.at(item.first)) {
      kb_word_reg_weight_for_child_[item.first] += std::max(mean_[kb_child_word_idx], float(0)); 
    }
  }
}
